...
export const sortArrayHandler = (arr, getter, order = 'asc') =>
  ...
  export const sendOrderDataToAPI = (orderData) => {
    return new Promise((resolve, reject) => {
      setTimeout(async () => {
        try {
          const response = await fetch(`${originAPi}/stripe/myBagData`, {
            method: "POST",
            headers: {
          ...
          reject(new Error(error.message || "Error sending order data"));
        }
      }, 3000); // 3-second delay
    });
  };
...
export async function OrderPlaced({ order }) {
  ...
  try {
    ...
    if (data.status === 200) {
      ...
      // Log the order details including the order ID
      console.log("Order details including ID:", data.order);
      return data.order;
    } else if (data.status == 300) {
      DestoryAuth();
    } else {
      if (data?.data) {
        // Log alternative success response
        console.log("Alternative success data:", data.data);
        return data.data;
      } else {
        return false;
      }
