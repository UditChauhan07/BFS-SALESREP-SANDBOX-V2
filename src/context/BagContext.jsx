...
const BagProvider = ({ children }) => {
  ...
  const addOrder = (product, quantity, discount) => {
    setOrders((prev) => {
      ...
      if (obj[productKey]) {
        ...
        obj[productKey]  = parseOrderObjectWithDiscount(
          product, quantity, discount, Object.values(obj)?.[0]?.account, Object.values(obj)?.[0]?.manufacturer, obj[productKey].id // Keep the same ID
        );
      } else {
       
        const uniqueId = generateRandomString()
        obj[productKey] = parseOrderObjectWithDiscount(
          product, quantity, discount, null, null, uniqueId // Pass unique ID
        );
      }
  ...
  const parseOrderObjectWithDiscount = (product, quantity, discount,account,manufacturer , id) => {
    return {
      uniqueId : id, 
      quantity: quantity,
      product,
