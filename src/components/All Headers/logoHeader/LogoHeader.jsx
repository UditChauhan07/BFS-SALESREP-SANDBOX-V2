...
import { sendOrderDataToAPI } from "../../../lib/store";
import "./style.css";
...
const LogoHeader = () => {
  ...
  const fetchOrdersAndSendToAPI = async () => {
    const orderData = localStorage.getItem("bagValues");
    ...
    try {
      ...
      const result = await sendOrderDataToAPI(parsedOrderData); // Use the fetch-based API function
      console.log("Order data sent successfully:", result);
    } catch (err) {
